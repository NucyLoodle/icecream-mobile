...
const TrackVan: React.FC = () => {
	...
	const stopSharing = async () => {
		...
		if (ws && ws.readyState === WebSocket.OPEN) {
			try {
				ws.send(JSON.stringify({ vanId, type: "vanStopped" }));
			} catch (error) {
				console.error("Error sending vanStopped payload:", error);
			}
			...
			console.warn("WebSocket not open or already closed");
		}

		console.log("Van ID being passed to checkOutVan:", vanId);
		await checkOutVan(vanId as string);
	};
